...
class WeedExtractor:
    def __init__(self, camera_frame):
        ...
        self.arrow_pub = rospy.Publisher("arrow", Marker, queue_size=10)
        self.src_pub = rospy.Publisher("source", PointCloud2, queue_size=10)
        self.inliers_pub = rospy.Publisher("inliers", PointCloud2, queue_size=10)
        self.plane_pub = rospy.Publisher("plane", PointCloud2, queue_size=10)

        rospy.Subscriber("/rviz_selected_points", PointCloud2, self.select_branch)
    ...
    def rviz_arrow(self, pub, start, direction, name, thickness=0.004, length_scale=0.2, color='r'):
        """
    ...
    def plot_plane(self, pub, centroid, normal, size: float = 1, res: float = 0.01):
        """
    ...
    def select_branch(self, selection):
        ...
        while True:
            ...
            self.plot_pointcloud_rviz(self.src_pub, points[:, 0], points[:, 1], points[:, 2])
            self.plot_pointcloud_rviz(self.inliers_pub, inlier_points[:, 0], inlier_points[:, 1], inlier_points[:, 2])
            rospy.sleep(1)
